...
/**
 * @ngdoc function
 * @name getlancerApp.controller:OrderPaymentsController
 * @description
 * # OrderPaymentsController
 * Controller of the getlancerApp
 */
angular.module('getlancerApp.Quote')
    .controller('OrderPaymentsController', ['$rootScope', '$scope', '$window', 'countries', 'states', 'cities', 'flash', '$location', '$filter', '$state', 'paymentGateways', 'UserMeFactory', 'PaymentOrderFactory', '$uibModalStack', 'ConstDiscountType', 'CouponGetStatusFactory', 'ConstPaymentGateways', function($rootScope, $scope, $window, countries, states, cities, flash, $location, $filter, $state, paymentGateways, UserMeFactory, PaymentOrderFactory, $uibModalStack, ConstDiscountType, CouponGetStatusFactory, ConstPaymentGateways) {
        ...
        $scope.plan_info.price_final = $scope.quote_request_views_detail.quote_amount;
        ...
        $scope.applyCoupon = function() {
            ...
            params.amount = $scope.quote_request_views_detail.quote_amount;
            CouponGetStatusFactory.get(params, function(response) {
                ...
                if (response.error.code === 0) {
                    if ($scope.discountCoupon.data.discount_type_id == ConstDiscountType.Amount) {
                        $scope.plan_info.price_final = parseFloat($scope.quote_request_views_detail.quote_amount) - parseFloat(response.data.discount);
                        $scope.show_discount = $filter("currency")(response.data.discount);
                    } else {
                        var discount_amt = ((response.data.discount / 100) * $scope.quote_request_views_detail.quote_amount);
                        $scope.plan_info.price_final = $scope.quote_request_views_detail.quote_amount - discount_amt;
        ...
        $scope.PaymentFormSubmit = function(form) {
            ...
            $scope.buyer.foreign_id = $scope.quote_request_views_detail.id;
            $scope.buyer.class = 'QuoteBid';
