 ...
 * @version		1.0
 * @author 		siva_063at09
 * @copyright 	Copyright (c) 2013 {@link http://www.agriya.com/ Agriya Infoway}
...
class ZazPay_API
{
    ...
    {
        $this->cache_path = dirname(dirname(dirname(dirname(dirname(dirname(dirname(__FILE__))))))) . DIRECTORY_SEPARATOR . 'tmp' . DIRECTORY_SEPARATOR . 'cache' . DIRECTORY_SEPARATOR;
        ...
        $this->api_v2_url = ($this->is_test) ? $this->sandbox_v2_api_url : $this->live_v2_api_url;
    }
    ...
    private function _execute($url, $method = 'get', $post = array() , $print_raw_response = false)
    {
        ...
        if ($print_raw_response) {
            // Pricing response of raw ZazPay; For few payment gateway IPN's we need to respond like "*ok*", "success" / "fail"; So we echo here what ZazPay response
            echo $response;
            exit;
        }
        ...
        case 400:
        case 500:
        case 504: /* Here we're not sure if anything got already triggered/saved in ZazPay */
            $return['error']['code'] = 1;
